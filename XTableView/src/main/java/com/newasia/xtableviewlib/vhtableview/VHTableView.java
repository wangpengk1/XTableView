...
public class VHTableView extends LinearLayout implements HListViewScrollView.ScrollChangedListener{
    ...
    private boolean mFixHeight = true;
    ...
    public void setFixHeight(boolean bTure)
    {
        mFixHeight = bTure;
    }
    ...
    public void setAdapter(final VHBaseAdapter conentAdapter) {
        ...
        cleanup();
        //载入标题行
        if(conentAdapter.getContentColumn()>0)
        initTitles(conentAdapter);
        //载入表格正文
        if(conentAdapter.getContentRows()>0)
        {
    ...
    private void initTitles(VHBaseAdapter conentAdapter) {
        ...
        int i=0;
        int maxheight = getTitleMaxHeight(conentAdapter);
...
//            ll_firstcolumn.addView(view, LayoutParams.WRAP_CONTENT,LayoutParams.WRAP_CONTENT);
            ll_firstcolumn.addView(view, view.getMeasuredWidth(),maxheight);
            ...
            for(;i<conentAdapter.getContentColumn();i++){
                ...
                int h = MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED);
                view.measure(0,0);
                ll_datagroup.addView(view,view.getMeasuredWidth(),maxheight);
    ...
    private int getTitleMaxHeight(VHBaseAdapter conentAdapter)
    {
        int maxHeight = 0;
        for (int i=0;i<conentAdapter.getContentColumn();++i)
        {
            View view = conentAdapter.getTitleView(i, null);
            int w = MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED);
            int h = MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED);
            view.measure(w, h);
    ...
    private void initContentList(VHBaseAdapter conentAdapter) {
        ...
        {
            addView(listView, LayoutParams.MATCH_PARENT, adapter.totalHeight());
        }
    ...
    public class  ContentAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, final ViewGroup parent) {
            ...
            mHeights.put(position,maxHeight);
            //重新更新view到表格上
